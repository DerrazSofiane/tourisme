...
def valeur_trimestrielle(fichier, annee):
    valeur = pd.DataFrame()
    annee_map = fichier["Semaine"].map(lambda x: x.year) == annee
    tableau_annee_mois = fichier[annee_map]
    tableau_annee_mois[str(annee)] = list(tableau_annee_mois.reset_index().index)
    
    return tableau_annee_mois
    
def moyenne_trimestrielle(fichier, annee, top6_trimestre):
    """ Fonction permettant de retourner un tableau des moyennes du trimestre
    T1 de l'année X par rapport a l'année X-1
    ...
    def boucle_mois(annee):
        ...
        for i in range(0,3):
            mois_map = fichier["Semaine"].map(lambda x: x.month) == 1+i
            tableau_mois = fichier[mois_map]
