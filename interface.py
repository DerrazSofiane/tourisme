    ...
    try:
        ...
        if st.sidebar.checkbox("2- Volumes brutes des 3 dernières années du top 6"):
            def top_last_annee(recap):
                ...
                for p in pays:
                    st.write(p)
                    annee1 = evolution_annee[p][(evolution_annee["annee"] == annees[0])].reset_index().drop("index", axis=1)
                    annee2 = evolution_annee[p][(evolution_annee["annee"] == annees[1])].reset_index().drop("index", axis=1)
                    annee3 = evolution_annee[p][(evolution_annee["annee"] == annees[2])].reset_index().drop("index", axis=1)
                    last = pd.concat([annee1, annee2, annee3], axis=1)
                    last.columns = [p+" " +annees[0], p+" "+annees[1], 
                                    p+" "+annees[2]]
